...
- (IBAction)productListButtonClicked:(id)sender { // no parameters
  [XKRouter pushToUrl:PRODUCT_LIST_PATH sourceViewController:self parameters:nil];
}

- (IBAction)productDetailButtonClicked:(id)sender { // 参数来源于本地
  [XKRouter pushToUrl:PRODUCT_DETAIL_PATH sourceViewController:self parameters:@{@"productId" : @"300432", @"productName" : @"Cherry MX8.0 机械键盘（青轴）"}];
}
...
- (IBAction)productApplyButtonClicked:(id)sender { // 参数来源于 url
  ...
  [XKRouter pushToUrl:[NSString stringWithFormat:@"%@?productId=11111&productPrice=15元&payWay=支付宝", PRODUCT_APPLY_PATH] sourceViewController:self parameters:nil];
}

- (IBAction)payButtonClicked:(id)sender { // 参数 = url + 本地
  [XKRouter pushToUrl:[NSString stringWithFormat:@"%@?productId=22222&productPrice=100刀", PRODUCT_APPLY_PATH] sourceViewController:self parameters:@{@"payWay" : @"Apply Pay"}];
}
